...
kubectl create secret generic ${DAEMON_NAME}-fts-x509up --from-file=x509up=/tmp/x509up_u0
...
kubectl create secret generic ${DAEMON_NAME}-hermes-x509up --from-file=x509up=/tmp/x509up_u0
...
kubectl create secret generic ${SERVER_NAME}-usercert-with-key --from-file=usercert_with_key.pem=/$(pwd)/host-cert-k8s-v3/usercert_with_key.pem
kubectl create secret generic ${UI_NAME}-usercert-with-key --from-file=usercert_with_key.pem=/$(pwd)/host-cert-k8s-v3/usercert_with_key.pem
