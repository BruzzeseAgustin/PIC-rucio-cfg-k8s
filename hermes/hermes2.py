...
def submit_to_elastic(messages, endpoint, prepend_str):
    ...
    elastic_server = config_get('hermes', 'elastic_server', False, None)
    # print(elastic_server)
    # Get desired nidex name form config - "rucio"
    elastic_index = config_get('hermes', 'elastic_index', False, None)
    # print(elastic_index)
    # Get date pattern from config (python date pattern) - "%Y.%m.%d"
    elastic_date_pattern = config_get('hermes', 'elastic_date_pattern', False, None)
    # print(elastic_date_pattern)
    # Set date
    elastic_date = "" if elastic_date_pattern == "" else datetime.datetime.now().strftime(elastic_date_pattern)
    print(elastic_date)
    # Build the entire URL    
    endpoint = elastic_server + "/" + elastic_index + elastic_date + "/_bulk"
    # print(endpoint)
    res = requests.post(endpoint, data=text, headers={'Content-Type': 'application/json'})
    return res.status_code
