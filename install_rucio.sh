...
helm install rucio bitnami/postgresql -f postgres/values.yaml
kubectl apply -f ./postgres/storage.yaml

PSQL_POD=$(kubectl get pods rucio-postgresql-0 -n prod-rucio-test | tail -n +2 | awk '{print $3}')

while [[ $PSQL_POD != "Running" ]]; do
  echo 'Wait until the postgres db pod iniciates...' && PSQL_POD=$(kubectl get pods rucio-postgresql -n prod-rucio-test | tail -n +2 | awk '{print $3}') && CR_STATE=$(kubectl get pods rucio-postgresql -n prod-rucio-test | tail -n +2 | awk '{print $3}') && echo "Current state is $CR_STATE" && sleep 1;
done
...
kubectl apply -f rucio-ui-nodeport.yaml
