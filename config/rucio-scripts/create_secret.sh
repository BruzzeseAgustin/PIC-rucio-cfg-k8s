...
if [ ! -f /$(pwd)/certs/robotcert.pem ] && [ -f /$(pwd)/certs/robotcert.pem ]; then
   yes | cp -rf /etc/pki/tls/certs/ca-bundle.trust.crt /$(pwd)/${DIR_CAS}/ca.pem
fi
...
kubectl create secret generic ${SERVER_NAME}-cafile --from-file=ca.pem=/$(pwd)/${DIR_CAS}/ca.pem
...
kubectl create secret generic ${SERVER_NAME}-auth-cafile --from-file=ca.pem=/$(pwd)/${DIR_CAS}/ca.pem
kubectl create secret generic ${SERVER_NAME}-rucio-x509up --from-file=x509up=/$(pwd)/${DIR_PROXY}/x509
kubectl create secret generic ${SERVER_NAME}-certs --from-file=/$(pwd)/${DIR_CAS}/ca.pem
...
kubectl create secret generic ${DAEMON_NAME}-fts-x509up --from-file=x509up=/$(pwd)/${DIR_PROXY}/x509
...
kubectl create secret generic ${DAEMON_NAME}-hermes-x509up --from-file=x509up=/$(pwd)/${DIR_PROXY}/x509
kubectl create secret generic ${DAEMON_NAME}-rucio-ca-bundle --from-file=ca-volume=/$(pwd)/${DIR_CAS}/ca.pem
kubectl create secret generic ${DAEMON_NAME}-hermes2-script --from-file=/$(pwd)/dependencies/hermes/

kubectl create secret generic ${DAEMON_NAME}-rucio-x509up --from-file=x509up=/$(pwd)/${DIR_PROXY}/x509
kubectl create secret generic ${DAEMON_NAME}-rucio-x509up-reaper --from-file=x509up=/$(pwd)/${DIR_PROXY}/x509
kubectl create secret generic ruciod-release-rucio-x509up --from-file=proxy-volume=/$(pwd)/${DIR_PROXY}/x509
...
cp /$(pwd)/${DIR_CAS}/grid-security/certificates/*.0 /tmp/reaper-certs/
cp /$(pwd)/${DIR_CAS}/grid-security/certificates/*.signing_policy /tmp/reaper-certs/
...
cp /$(pwd)/${DIR_CAS}/grid-security/certificates/*.0 /tmp/user-certs/
...
kubectl create secret generic ${UI_NAME}-cafile  --from-file=ca.pem=/$(pwd)/${DIR_CAS}/ca_full.pem
...
kubectl create secret generic ${SERVER_NAME}-rucio-ca-bundle --from-file=/$(pwd)/${DIR_CAS}/ca.pem
