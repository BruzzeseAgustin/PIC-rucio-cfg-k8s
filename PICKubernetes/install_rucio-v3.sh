...
while [[ $PSQL_POD != "Running" ]]; do
  echo 'Wait until the postgres db pod iniciates...' && CR_STATE=$(kubectl get pods postgres-postgresql-0 -n prod-rucio-test | awk '{print $3}') && echo "Current state is $CR_STATE" && sleep 1;
done
...
while [[ $INIT_POD != "Completed" ]]; do 
  echo 'Iniciating rucio db with basic user...' && CR_STATE=$(kubectl get pods test-init -n prod-rucio-test | awk '{print $3}') && echo "Current state is $CR_STATE" && sleep 1;
done
