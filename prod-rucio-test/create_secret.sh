...
kubectl create secret tls rucio-server.tls-secret --key=/$(pwd)/host-cert-k8s-v4/hostkey.pem --cert=/$(pwd)/host-cert-k8s-v4/hostcert.pem
kubectl create secret tls rucio-web.tls-secret --key=/$(pwd)/host-cert-web-v2/hostkey.pem --cert=/$(pwd)/host-cert-web-v2/hostcert.pem

kubectl create secret generic pic01-server-hostcert --from-file=hostcert.pem=/$(pwd)/host-cert-k8s-v4/hostcert.pem
kubectl create secret generic pic01-server-hostkey --from-file=hostkey.pem=/$(pwd)/host-cert-k8s-v4/hostkey.pem
kubectl create secret generic pic01-cafile --from-file=ca.pem=/$(pwd)/host-cert-k8s-v4/pic01-rucio-server_pic_es_interm.cer 

kubectl create secret generic pic01-hostcert --from-file=hostcert.pem=/$(pwd)/host-cert-k8s-v4/hostcert.pem
kubectl create secret generic pic01-hostkey --from-file=hostkey.pem=/$(pwd)/host-cert-k8s-v4/hostkey.pem
kubectl create secret generic pic01-cafile --from-file=ca.pem=/$(pwd)/host-cert-k8s-v4/pic01-rucio-server_pic_es_interm.cer 

kubectl create secret generic pic01-auth-hostcert --from-file=hostcert.pem=/$(pwd)/host-cert-k8s-v4/hostcert.pem
kubectl create secret generic pic01-auth-hostkey --from-file=hostkey.pem=/$(pwd)/host-cert-k8s-v4/hostkey.pem
kubectl create secret generic pic01-auth-cafile --from-file=ca.pem=/$(pwd)/host-cert-k8s-v4/pic01-rucio-server_pic_es_interm.cer 

kubectl create secret generic pic01-trace-hostcert --from-file=hostcert.pem=/$(pwd)/host-cert-k8s-v4/hostcert.pem
kubectl create secret generic pic01-trace-hostkey --from-file=hostkey.pem=/$(pwd)/host-cert-k8s-v4/hostkey.pem
kubectl create secret generic pic01-trace-cafile --from-file=ca.pem=/$(pwd)/host-cert-k8s-v4/pic01-rucio-server_pic_es_interm.cer 
...
kubectl create secret generic clients-rucio-ca-bundle-test --from-file=ca.pem=/$(pwd)/host-cert-k8s-v4/pic01-rucio-server_pic_es_interm.cer 

kubectl create secret generic clients-admin-cert --from-file=usercert.pem=/$(pwd)/host-cert-k8s-v4/hostcert.pem
kubectl create secret generic clients-admin-key --from-file=userkey.pem=/$(pwd)/host-cert-k8s-v4/hostkey.pem

kubectl create secret generic server-server-hostcert --from-file=hostcert.pem=/$(pwd)/host-cert-k8s-v4/hostcert.pem
kubectl create secret generic server-server-hostkey --from-file=hostkey.pem=/$(pwd)/host-cert-k8s-v4/hostkey.pem
# kubectl create secret generic server-server-cafile --from-file=ca.pem=/$(pwd)/host-cert-k8s-v4/rucio03_pic_es_interm.cer
...
kubectl create secret generic ${DAEMON_NAME}-fts-cert --from-file=usercert=/$(pwd)/host-cert-k8s-v4/hostcert.pem
kubectl create secret generic ${DAEMON_NAME}-fts-key --from-file=userkey=/$(pwd)/host-cert-k8s-v4/hostkey.pem
kubectl create secret generic ${DAEMON_NAME}-hermes-cert --from-file=usercert=/$(pwd)/host-cert-k8s-v4/hostcert.pem
...
export UI_NAME=web
kubectl create secret generic ${UI_NAME}-hostcert --from-file=hostcert.pem=/$(pwd)/host-cert-web-v2/hostcert.pem
kubectl create secret generic ${UI_NAME}-hostkey --from-file=hostkey.pem=/$(pwd)/host-cert-web-v2/hostkey.pem
kubectl create secret generic ${UI_NAME}-cafile --from-file=ca.pem=/$(pwd)/host-cert-web-v2/pic01-rucio-server-web_pic_es_interm.cer
